...
abstract class Trend extends RangedMetric
{
        ...
        switch ($unit) {
            ...
            case 'day':
                return with(Carbon::createFromFormat('Y-m-d', $result), function ($date) {
                    return Nova::__($date->format('F')) . ' Trend.php' .$date->format('j').', '.$date->format('Y');
                });

            case 'hour':
                return with(Carbon::createFromFormat('Y-m-d H:00', $result), function ($date) use ($twelveHourTime) {
                    return $twelveHourTime
                            ? Nova::__($date->format('F')) . ' Trend.php' .$date->format('j').' - '.$date->format('g:00 A')
                            : Nova::__($date->format('F')) . ' Trend.php' .$date->format('j').' - '.$date->format('G:00');
                });
            ...
            default:
                return with(Carbon::createFromFormat('Y-m-d H:i:00', $result), function ($date) use ($twelveHourTime) {
                    return $twelveHourTime
                            ? Nova::__($date->format('F')) . ' Trend.php' .$date->format('j').' - '.$date->format('g:i A')
                            : Nova::__($date->format('F')) . ' Trend.php' .$date->format('j').' - '.$date->format('G:i');
                });
        }
    }
    ...
    protected function formatAggregateMonthDate($result)
    {
        ...
        return with(Carbon::create((int) $year, (int) $month, 1), function ($date) {
            return Nova::__($date->format('F')) . ' Trend.php' .$date->format('Y');
        });
    }
    ...
    protected function formatAggregateWeekDate($result)
    {
        ...
        return Nova::__($startingDate->format('F')) . ' Trend.php' .$startingDate->format('j').' - '.
    ...
    protected function formatPossibleAggregateResultDate(CarbonInterface $date, $unit, $twelveHourTime)
    {
        switch ($unit) {
            ...
            case 'week':
                return Nova::__($date->startOfWeek()->format('F')) . ' Trend.php' .$date->startOfWeek()->format('j').' - '.
